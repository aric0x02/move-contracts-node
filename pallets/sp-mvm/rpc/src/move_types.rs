...
impl TryFrom<AnnotatedMoveStruct> for MoveResource {
	...
	fn try_from(s: AnnotatedMoveStruct) -> anyhow::Result<Self> {
		Ok(Self { typ: s.type_.clone().into(), data: s.try_into()? })
	}
}
...
impl FromStr for HexEncodedBytes {
	...
	fn from_str(s: &str) -> anyhow::Result<Self, anyhow::Error> {
		let hex_str = if let Some(hex) = s.strip_prefix("0x") { hex } else { s };
		Ok(Self(hex::decode(hex_str).map_err(|e| {
			format_err!("decode hex-encoded string({:?}) failed, caused by error: {}", s, e)
		})?))
	}
}
...
impl From<HexEncodedBytes> for move_core_types::value::MoveValue {
	fn from(d: HexEncodedBytes) -> Self {
		move_core_types::value::MoveValue::Vector(
			d.0.into_iter().map(move_core_types::value::MoveValue::U8).collect(),
		)
	}
}
		...
		if let Some((_, AnnotatedMoveValue::Bytes(bytes))) = v.value.into_iter().next() {
			match String::from_utf8(bytes.clone()) {
				...
				Err(_) => {
					...
					// conversion
					Ok(MoveValue::String(format!("Unparsable utf-8 {}", HexEncodedBytes(bytes))))
				},
			}
		} else {
	...
	fn try_from(val: AnnotatedMoveValue) -> anyhow::Result<Self> {
		Ok(match val {
			...
			AnnotatedMoveValue::Vector(_, vals) => MoveValue::Vector(
				vals.into_iter().map(MoveValue::try_from).collect::<anyhow::Result<_>>()?,
			),
...
impl VerifyInputWithRecursion for MoveStructTag {
	fn verify(&self, recursion_count: u8) -> anyhow::Result<()> {
		...
		verify_module_identifier(self.module.0.as_str())
			.map_err(|_| anyhow::anyhow!("invalid struct tag: {}", self))?;
		verify_identifier(self.name.0.as_str())
			.map_err(|_| anyhow::anyhow!("invalid struct tag: {}", self))?;
		for param in self.generic_type_params.iter() {
			param.verify(recursion_count + 1).map_err(|err| {
				anyhow::anyhow!("Invalid struct tag for generic type params: {} {}", self, err)
			})?;
		}
...
impl MoveStructTag {
	pub fn new(
		...
		Self { address, module, name, generic_type_params }
	}
}
...
impl FromStr for MoveType {
	...
	fn from_str(mut s: &str) -> Result<Self, Self::Err> {
		...
		if is_ref {
			Ok(MoveType::Reference { mutable: is_mut, to: Box::new(inner) })
		} else {
			Ok(inner)
		}
	}
...
impl From<TypeTag> for MoveType {
	fn from(tag: TypeTag) -> Self {
		match tag {
			...
			TypeTag::Vector(v) => MoveType::Vector { items: Box::new(MoveType::from(*v)) },
...
impl From<&TypeTag> for MoveType {
	fn from(tag: &TypeTag) -> Self {
		match tag {
			...
			TypeTag::Address => MoveType::Address,
			TypeTag::Signer => MoveType::Signer,
			TypeTag::Vector(v) => MoveType::Vector { items: Box::new(MoveType::from(v.as_ref())) },
			TypeTag::Struct(v) => MoveType::Struct(v.into()),
		}
	}
}
...
impl From<CompiledModule> for MoveModule {
	fn from(m: CompiledModule) -> Self {
		println!("from(m: CompiledModule)=result==={:?}===", 747);
		let (address, name) = <(AccountAddress, Identifier)>::from(m.self_id());
		Self {
			address: address.into(),
			name: name.into(),
			friends: m.immediate_friends().into_iter().map(|f| f.into()).collect(),
			exposed_functions: m
				.function_defs
				.iter()
				...
				.collect(),
			structs: m.struct_defs.iter().map(|def| m.new_move_struct(def)).collect(),
		}
	}
}
...
impl From<&AbilitySet> for MoveFunctionGenericTypeParam {
	fn from(constraints: &AbilitySet) -> Self {
		Self { constraints: constraints.into_iter().map(MoveAbility::from).collect() }
	}
}
...
impl MoveModuleBytecode {
	pub fn new(bytes: Vec<u8>) -> Self {
		Self { bytecode: bytes.into(), abi: None }
	}

	pub fn try_parse_abi(mut self) -> anyhow::Result<Self> {
		if self.abi.is_none() {
			println!("try_parse_abi=result==={:?}===", 1072);
			...
			if let Ok(module) = CompiledModule::deserialize(self.bytecode.inner()) {
				println!("try_parse_abi=result==={:?}===", 1077);
...
impl VerifyInput for EntryFunctionId {
	fn verify(&self) -> anyhow::Result<()> {
		self.module.verify().map_err(|_| invalid_entry_function_id(self))?;
		self.name.verify().map_err(|_| invalid_entry_function_id(self))
	}
}
...
pub fn verify_identifier(identifier: &str) -> anyhow::Result<()> {
	if identifier.contains("::") {
		Err(format_err!("Identifier should not contain '::' {}", identifier))
	} else {
		Ok(())
	}
}
...
mod tests {
	...
	fn test_serialize_move_resource() {
		...
		let res = MoveResource::try_from(annotated_move_struct(
			...
			vec![
				...
				(identifier("field_vector"), Vector(TypeTag::U128, vec![U128(128)])),
	...
	fn test_serialize_move_resource_with_address_0x0() {
		let res = MoveResource::try_from(annotated_move_struct(
			"Values",
			vec![(identifier("address_0x0"), AnnotatedMoveValue::Address(address("0x0").into()))],
		))
		.unwrap();
		let value = to_value(&res).unwrap();
		assert_json(
	...
	fn test_serialize_deserialize_move_module_id() {
		test_serialize_deserialize(
			MoveModuleId { address: "0x1".parse().unwrap(), name: "Aptos".parse().unwrap() },
			json!("0x1::Aptos"),
		);
	}
	...
	fn test_parse_invalid_move_module_id_string() {
		...
		assert_eq!(
			...
			"Aptos::Aptos".parse::<MoveModuleId>().err().unwrap().to_string()
		);
		assert_eq!(
			"Invalid Move module ID: 0x1::Aptos::Aptos",
			"0x1::Aptos::Aptos".parse::<MoveModuleId>().err().unwrap().to_string()
		);
	}
	...
	fn test_parse_invalid_move_entry_function_id_string() {
		...
		assert_eq!(
			...
			"Aptos::Aptos".parse::<EntryFunctionId>().err().unwrap().to_string()
		);
		assert_eq!(
			"Invalid entry function ID Aptos::Aptos::??",
			"Aptos::Aptos::??".parse::<EntryFunctionId>().err().unwrap().to_string()
		...
		assert_eq!(
			...
			"0x1::Aptos::Aptos::Aptos".parse::<EntryFunctionId>().err().unwrap().to_string()
		);
	}
	...
	fn annotated_move_struct(
		...
		AnnotatedMoveStruct { abilities: AbilitySet::EMPTY, type_: type_struct(typ), value: values }
	}
