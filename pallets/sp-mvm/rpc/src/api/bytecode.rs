	...
	fn new_move_type(&self, token: &SignatureToken) -> MoveType {
		match token {
			...
			SignatureToken::Vector(t) =>
				MoveType::Vector { items: Box::new(self.new_move_type(t.borrow())) },
			...
			SignatureToken::Reference(t) =>
				MoveType::Reference { mutable: false, to: Box::new(self.new_move_type(t.borrow())) },
			SignatureToken::MutableReference(t) =>
				MoveType::Reference { mutable: true, to: Box::new(self.new_move_type(t.borrow())) },
		}
	}
	...
	fn new_move_struct(&self, def: &StructDefinition) -> MoveStruct {
		...
		let (is_native, fields) = match &def.field_information {
			...
			StructFieldInformation::Declared(fields) =>
				(false, fields.iter().map(|f| self.new_move_struct_field(f)).collect()),
		};
		let name = self.identifier_at(handle.name).to_owned();
		let abilities = handle.abilities.into_iter().map(MoveAbility::from).collect();
		let generic_type_params =
			handle.type_parameters.iter().map(MoveStructGenericTypeParam::from).collect();
		MoveStruct { name: name.into(), is_native, abilities, generic_type_params, fields }
	}
